...
| --- | --- |
| System Ruby | [`MasterResumeStore`](../Sources/NavCenterCore/MasterResumeStore.swift) validates YAML through a bounded subprocess. |
| Pandoc, Chrome, Poppler | [`ArtifactExporter`](../Sources/NavCenterCore/ArtifactExporter.swift) produces HTML, DOCX, PDF and extracted text. Overrides are `PANDOC_BIN`, `CHROME_BIN` and `PDFTOTEXT_BIN`. Pandoc must support `--sandbox`; Chrome renders inert content using a separate profile and retains its sandbox. |
| External `atsim` | [`PackageActionRunner`](../Sources/NavCenterCore/PackageActionRunner.swift) resolves `NAV_CENTER_ATSIM_BIN` or `atsim` on PATH. A confirmed scan stages only required synthetic/user-selected package inputs and sets `ATSIM_JOB_HUNT_ROOT` to that staging workspace. A successful command must also produce a valid fresh report. |
| External exporter | `NAV_CENTER_EXPORT_BIN` can replace the built-in confirmed package export action with a compatible executable. |
...
| Codex | The app launches `codex app-server --listen stdio://`, optionally located through `DASHBOARD_CODEX_BIN`. Sign-in and package edit approval are separate. Requests and selected context are processed by the services used by the signed-in account; local stdio transport does not make model processing offline. |
