...
| --- | --- |
| System Ruby | [`MasterResumeStore`](../Sources/NavCenterCore/MasterResumeStore.swift) validates YAML through a bounded subprocess. Ruby is resolved on PATH, then in `/opt/homebrew/bin`, `/usr/local/bin`, and `~/.local/bin`. The probe checks for an executable regular file and never executes Ruby; the validator runs only after Ruby is found. |
...
| Codex | The app launches `codex app-server --listen stdio://`. Lookup order is `DASHBOARD_CODEX_BIN`, then PATH, then `/opt/homebrew/bin`, `/usr/local/bin`, and `~/.local/bin`. `ToolProbe` uses that order for diagnostics and never executes Codex. The app target still resolves the command inside [`NativeCodexBridge`](../Sources/NavCenterApp/Services/NativeCodexBridge.swift) until a later slice calls the probe. Sign-in and package edit approval are separate. Requests and selected context are processed by the services used by the signed-in account; local stdio transport does not make model processing offline. |
