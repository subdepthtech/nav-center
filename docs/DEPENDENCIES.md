...
| `vendor/atsim` | Unchanged review snapshot at `cc37c5b1e3a4f7dfe17d9f043eb18021ff6faef4`; 11 upstream file hashes; not built, bundled, or shipped | See [THIRD_PARTY_NOTICES.md](../THIRD_PARTY_NOTICES.md). The MIT license text was confirmed by the copyright holder on 2026-09-22 (© 2026 Austin Tucker). The `PENDING UPSTREAM CONFIRMATION` gate stays in the scripts for future snapshots: `--distribution` refuses to build while such a marker is present |
...
`python3 -B scripts/verify-vendor.py` checks both hashes and the exact snapshot file inventory, and requires [THIRD_PARTY_NOTICES.md](../THIRD_PARTY_NOTICES.md) to name `vendor/atsim` and the snapshot commit from `vendor/atsim/UPSTREAM.md`. While that notice contains `PENDING UPSTREAM CONFIRMATION`, the command reports the open distribution gate and still exits successfully. The current notice has no marker. A matching manifest is integrity evidence, not upstream security or license approval. Never let dependency bots rewrite the snapshot implicitly.
