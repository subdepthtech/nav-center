...
The prepared workflow selects macOS 26 and Xcode 26.6, restricts execution to `main`, and retains validated release evidence for 90 days. It records GitHub's dependency-graph SPDX inventory plus [dependency boundaries](DEPENDENCIES.md); this is a source dependency inventory, not an exhaustive binary SBOM. The `release` environment requires a human maintainer's approval. The signing and notarization secrets are provisioned in the `release` environment (2026-09-23; see U2 in [MACOS-BETA-MILESTONE.md](MACOS-BETA-MILESTONE.md)); an explicitly authorized workflow run and verification of its artifact remain separate gates; see [SETUP.md](SETUP.md).
