...
- `navcenterctl feedback-diagnostics` is unredacted unless `--redact` is passed, and `--redact` drops `recentLogs` entirely, so the per-line log redactor is unreachable and untested. There is no GUI entry point for diagnostics.
