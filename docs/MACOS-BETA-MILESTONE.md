...
- Tracker history is never displayed: `NativeDashboardService` hardcodes `statusEvents: []` and no view reads `PackageResponse.statusEvents`. A status change made from the Applications table shows no confirmation (the success text renders only in the selected package's rail), and import errors render as a grey caption rather than an alert.
- `navcenterctl feedback-diagnostics` is unredacted unless `--redact` is passed. The redactor rewrites home and user paths, not log content, so dropping `recentLogs` under `--redact` is correct and must stay. There is no GUI entry point for diagnostics.
- A missing Ruby surfaces on master-resume save as "Master resume YAML is invalid: …" because `MasterResumeStore` spawns bare `ruby` and reports its stderr.
- The May release's `.sha256` sidecar records `dist/NavCenter-0.1.0-beta-macos-arm64.dmg` (path-prefixed); the release-evidence verifier requires a bare basename, so re-verifying that artifact fails on the sidecar, not the bytes. `plugins/nav-center/.codex-plugin/plugin.json` pins `"version": "0.1.0-beta"` with no link to the DMG version scheme. A local `package-beta-dmg.sh --distribution` runs `spctl` on the staging copy; only the workflow assesses the mounted DMG.
- The atsim upstream repository has no LICENSE file at HEAD either (GitHub `/license` and both LICENSE paths return 404 on 2026-09-22), not only at the snapshot commit.
...
### WP3 — Bounded reads and dead paths (A5) — S/M, deferrable

Deferrable: if schedule pressure appears, this WP may slip past the beta without blocking WP11, unless WP12 surfaces a stall on a large file. It is kept because a single oversized `posting.md` is read on every refresh for every package.
...
### WP6 — Codex automation outcome, tracker history display, Interview tab truth (A6) — M

- Outcome: creating a package with Codex automation never silently drops the automation; a tester can see a package's status history after a status change and gets a visible confirmation wherever the change was made; import failures are surfaced as errors; the Interview tab makes no claims about a model or API key the app does not use.
- Scope: `sendCodexMessage` returns `CodexSendOutcome` (started/busy/rejected/failed); `createPackageFromIntake` publishes `lastCodexAutomationOutcome` and sets `intakeMessage` to one of: "Created package: N. Codex is building the resume and prep files; watch the Codex panel." / "Created package: N. Codex automation did not start because another Codex turn is running. Open the package and send the build prompt from the Codex panel." / "Created package: N. Codex automation failed: <reason>". Tracker history: `NativeDashboardService` populates `PackageResponse.statusEvents` from `status_events` for the package (bounded, newest first, via `TrackerStore`), `PackageDetailView` renders a "Status History" list (timestamp, old → new status) with accessibility labels, and the status-change success text is shown as a transient banner in `ApplicationsView` as well as in the package rail; import errors from `DashboardStore` use the existing error alert instead of a caption. Interview tab: replace the model metric with "Kit Format: Local JSON (for an external realtime client)" and the API-key sentence with "Nav Center writes interview-realtime-session.json for an external realtime interview client. It does not call any model API and never stores API keys."
...
- Tests: `DashboardParityTests.testIntakeReportsBusyCodexInsteadOfSilentlyDroppingAutomation`, `testIntakeReportsCodexFailureAfterPackageIsCreated`, `testIntakeWithoutAutomationLeavesOutcomeNotRequested`, `testPackageResponseCarriesStatusEventsFromFakeService`, `testStatusChangeFromTablePublishesConfirmationMessage`; `UXReadinessTests.testStatusHistoryListsEventsNewestFirstAfterStatusChange` (real `TrackerStore` on a disposable workspace), `testImportFailureSurfacesAsError`.
- Evidence: PR CI.
- Autonomy: autonomous.
...
- Gates (2026-09-23 sequencing correction): WP11 may build the signed candidate before WP7/WP8 are recorded; this supersedes the WP7 → WP11 and WP8 → WP11 build-order edges in §4, which remain sharing gates. WP7, WP8, the WP11 downloaded-artifact verification, and WP12A must pass before the first tester. WP12B must pass before the tap route is advertised or the cohort grows beyond one. The cohort then grows toward the U5 maximum of 10 only while evidence stays clean.
