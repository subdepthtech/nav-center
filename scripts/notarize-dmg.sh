...
NOTARY_FILE=""
DMG_DIR=""
retain_notary_response() {
  local diagnostic
  diagnostic="$(mktemp "$DMG_DIR/.$(basename "$DMG_PATH").notary-failure.XXXXXX")" || return 1
  if mv "$NOTARY_FILE" "$diagnostic"; then
    NOTARY_FILE=""
    echo "Notarization submission response retained at: $diagnostic" >&2
    return 0
  fi
  rm -f "$diagnostic"
  return 1
...
cleanup() {
  local status=$?
  rm -f "$KEY_FILE"
  if [[ -n "$CHECKSUM_FILE" ]]; then rm -f "$CHECKSUM_FILE"; fi
  if [[ -n "$NOTARY_FILE" ]]; then
    if [[ $status -ne 0 && -n "$DMG_DIR" ]]; then
      retain_notary_response || rm -f "$NOTARY_FILE"
    else
      rm -f "$NOTARY_FILE"
    fi
  fi
...
NOTARY_FILE="$(mktemp "$DMG_DIR/.notary.XXXXXX")"
xcrun notarytool submit "$DMG_PATH" \
  --key "$KEY_FILE" \
  --key-id "$APP_STORE_CONNECT_KEY_ID" \
  --issuer "$APP_STORE_CONNECT_ISSUER_ID" \
  --wait --timeout 20m --output-format json >"$NOTARY_FILE"
[[ "$(plutil -extract status raw "$NOTARY_FILE")" == Accepted ]] || { echo "Notarization was not accepted." >&2; exit 1; }
mv "$NOTARY_FILE" "$DMG_PATH.notary.json"
NOTARY_FILE=""
